<?php 
	...
	if (!isset($_SESSION['tra_loi_de_thi_jlpt_nghe'])) {
		$_SESSION['tra_loi_de_thi_jlpt_nghe'] = array();
		$_SESSION['tra_loi_de_thi_jlpt_nghe'][] = array($cau_so, $chon);
	} else {
		...
		foreach ($_SESSION['tra_loi_de_thi_jlpt_nghe'] as $key => $val) {
			if ($val[0] == $cau_so) {
				$flag = 1;
				$_SESSION['tra_loi_de_thi_jlpt_nghe'][$key][1] = $chon;
			}
		}
		if ($flag == 0) {
			$_SESSION['tra_loi_de_thi_jlpt_nghe'][] = array($cau_so, $chon);
	...
	$bai = $action->getDetail('cau_thi_nghe', 'id', $cau_so);
