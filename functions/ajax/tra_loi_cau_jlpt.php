<?php 
	...
	if (!isset($_SESSION['tra_loi_de_thi_jlpt'])) {
		$_SESSION['tra_loi_de_thi_jlpt'] = array();
		$_SESSION['tra_loi_de_thi_jlpt'][] = array($cau_so, $chon);
	} else {
		...
		foreach ($_SESSION['tra_loi_de_thi_jlpt'] as $key => $val) {
			if ($val[0] == $cau_so) {
				$flag = 1;
				$_SESSION['tra_loi_de_thi_jlpt'][$key][1] = $chon;
			}
		}
		if ($flag == 0) {
			$_SESSION['tra_loi_de_thi_jlpt'][] = array($cau_so, $chon);
