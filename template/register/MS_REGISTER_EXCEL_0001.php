...
// var_dump($_SERVER['REMOTE_ADDR']);
    function lich_su_hoat_dong ($user_id) {
      global $conn_vn;
      
      $ip = $_SERVER['REMOTE_ADDR'];
      $ngay = date('Y-m-d H:i:s');

      $sql = "INSERT INTO lich_su_hoat_dong (user_id, ip, ngay) VALUES ('$user_id', '$ip', '$ngay')";
    ...
    function dangky () {
        ...
        if (isset($_POST['register'])) {
            ...
            if ($check == "true") {
                ...
                $sql = "INSERT INTO user (user_name, user_email, user_phone, user_birthday, user_password, time, ask, login_state, login_time) VALUES ('$name', '$email', '$phone', '$birthday', '$pass', '$time', '$ask', '1', '$login_time')";
                $result = mysqli_query($conn_vn, $sql);
                if ($result) {
            ...
            if ($num == 0) {
              ...
              ////////////////////////////////
                ...
                if ($row['login_state'] == 1) {
                  $tg = $row['login_time'];
                  $time_out = $login_time - $tg;
                  if ($time_out >= 600) {
                    
                  } else {
                    $message_login = "<div class='alert alert-danger'>Đang có người truy cập</div>";
                    return false;
                  }
                }

                $sql = "UPDATE user SET login_state = 1, login_time = '$login_time' WHERE user_email = '$email'";
                $result_login = mysqli_query($conn_vn, $sql);
                /////////////////////////////
                ...
                if (password_verify($pass, $pass_hash)) {
                    ...
                    lich_su_hoat_dong($_SESSION['user_id_gbvn']);
...
<?php 
  ...
  $huong_dan = $action->getDetail('page', 'page_id', 55);
?>
<script>
